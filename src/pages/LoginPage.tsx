...
const LoginPage = () => {
  ...
  const [selectedRole, setSelectedRole] = useState<'mechanic' | 'admin'>('mechanic');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (mode === 'signup') await signUpWithEmail(name.trim(), email.trim(), password, selectedRole);
      else await loginWithEmail(email.trim(), password, selectedRole);
    } catch (err: any) {
  ...
  const handleGoogle = async () => {
    setIsSubmitting(true); setError(null);
    try { await loginWithGoogle(selectedRole); }
    catch (err: any) {
        ...
        >
          ...
          <div className="bg-white rounded-2xl border border-border/60 shadow-sm p-6 space-y-4">
            ...
            <button type="button" onClick={handleGoogle} disabled={isSubmitting}
              className="w-full flex items-center justify-center gap-2.5 py-2.5 rounded-lg border border-border bg-white text-sm font-medium text-foreground hover:bg-gray-50 transition disabled:opacity-60">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] text-muted-foreground font-medium">or with email</span>
              <div className="flex-1 border-t border-border" />
            </div>
