...
import { fetchJobCardById, fetchInspectionReport, fetchJobPhotos } from '@/lib/firestore';
import { JobCard, InspectionReport } from '@/types';
import { 
  ChevronLeft, Car, Fuel, Gauge, Calendar, Clock, 
  User, FileText, AlertTriangle, CheckCircle, Loader2,
  ClipboardList, Printer, Image, Circle
} from 'lucide-react';
...
const statusClass: Record<JobCard['status'], string> = {
  ...
  test_drive: 'bg-purple-500/10 text-purple-600',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {!preServiceReport ? (
              ...
              <ReportCard report={preServiceReport} type="Pre-Service" photos={photos as Record<string, string>} />
            )}
          </div>
        </div>
      </div>
...
const ReportCard = ({ report, type, photos }: { report: InspectionReport; type: string; photos?: Record<string, string> }) => {
  ...
  return (
    ...
    >
            ...
            return (
              <div key={key} className={`rounded-lg p-3 border ${hasCondition ? 'bg-muted border-border' : 'bg-accent/5 border-accent/20'}`}>
                ...
                <p className={`text-sm font-medium ${hasCondition ? 'text-foreground' : 'text-accent italic'}`}>
                  {hasCondition ? value : 'Not recorded'}
                </p>
              </div>
            );
          })}
      ...
      <div className="mb-4">
        ...
        {showDamageMap && (
          <CarDiagram damages={report.damages} onAreaClick={() => {}} photos={photos as any} />
        )}
      </div>
